...
| `params` | `Record<string, unknown>` | — | — | External filters owned by the page. Merged into the `request` argument; a change refetches from page 1. Compared by value, so an inline object literal is safe |
| `refreshToken` | `string \| number` | — | — | Change it to refetch the **current** page without resetting to page 1 — for reloading after a mutation, a poll tick, or a cache invalidation |
...
| `pagination` | `{ defaultPageSize?: number; defaultCurrent?: number; pageSizeOptions?: number[]; onChange?: (page: number, pageSize: number) => void }` | — | — | Pagination config. `defaultCurrent` (1-based) sets the page to open on and is read once on mount; `onChange` fires on user paging only, never on mount — pair them to persist paging to the URL |
...
| `persistColumnVisibility` | `boolean \| string` | — | `true` | Persist show/hide column state to `localStorage`. `true` uses an auto-derived key; pass a `string` for an explicit key; `false` disables persistence |
...
### Filters and reloading

`request` is held in a ref, so replacing the function does **not** refetch. Filters that live
outside the table go through `params`; a "reload this" signal goes through `refreshToken`:
...
<ProTable<Label>
  ...
  rowKey="id"
  request={request}
  // A change here means a new result set → refetch from page 1.
  params={{ status, storeId }}
  // A change here means the same result set, freshly fetched → stay on the current page.
